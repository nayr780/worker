...
- Só abre WebSocket para o upstream DEPOIS de receber algum byte do cliente.
  Isso evita abrir WS para scanners de porta que conectam e fecham sem mandar nada.
...
LOG_LEVEL_NAME = os.getenv("REDIS_PROXY_LOG_LEVEL", "WARNING").upper()
log = logging.getLogger("redis-proxy")


def configure_logging() -> None:
    """
    Configura logging APENAS para o logger 'redis-proxy' quando o script é
    executado diretamente (python redis_ws_tcp_proxy.py).

    Quando o módulo é apenas importado (worker, client FastAPI, etc),
    NÃO chamamos isso, assim o logging global do app não é sobrescrito.
    """
    ...
    log.setLevel(level)
    # Em modo standalone não queremos que ele propague pro root e duplique log
    log.propagate = False
...
async def main():
    """
    Inicia o servidor TCP->WS usando LOCAL_HOST/LOCAL_PORT/WS_URL/TOKEN.
    Pode ser usado tanto como:

        python redis_ws_tcp_proxy.py

    quanto importado e rodado via:

        asyncio.create_task(redis_ws_tcp_proxy.main())
    """
